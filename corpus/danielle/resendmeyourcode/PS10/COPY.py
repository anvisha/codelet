...
class GreedyAdvisor(SubjectAdvisor):
    ...
    def pickSubjects(self, subjects, maxWork, maxLottery):
        ...
        a list of Subject instances that are chosen to take
        """
        #sorts subjects by the desired method
        sorted_Subjects = sorted(subjects, self.comparator)
...
class BruteForceAdvisor(SubjectAdvisor):
    ...
    def helper(self, subjects_left, work_avail,lot_avail, taken):
        if subjects_left == [] or work_avail == 0:
            result = (0,())
        elif subjects_left[0].getWork() > work_avail or lot_avail-subjects_left[0].getLottery()<0:
            result = self.helper(subjects_left[1:],work_avail,lot_avail, taken)
        else:
            subject = subjects_left[0]
            #This code adds the class to the subject and continues on
            withVal, withToTake = self.helper(subjects_left[1:],
                                         work_avail - subject.getWork(), lot_avail-subject.getLottery(),
                                         taken + [subject])
            withVal += subject.getValue()
            #This code doesn't take the class and sees what happens
            withoutVal, withoutToTake = self.helper(subjects_left[1:],
                                                                work_avail,lot_avail, taken)
...
class MemoizingAdvisor(SubjectAdvisor):
    ...
    def helper(self, subjects_left,work_avail, taken, lot_avail, memo):
        #creates dictionary called memo in there isn't one
        if memo==None:
            memo={}
        #small prob keeps track of problem variables to store for later
        small_prob=(len(subjects_left), work_avail, lot_avail)
        #if the problem with specific parameters has been encountered before
        #it brings the solution up from the memo dictionary
        if small_prob in memo:
            result=memo[small_prob]
            return result
        #This code is essentially the same as the previous code in the
        #brute force advisor
        if subjects_left == [] or work_avail == 0:
            result = (0,())
        elif subjects_left[0].getWork() > work_avail or lot_avail-subjects_left[0].getLottery()<0:
            result = self.helper(subjects_left[1:],work_avail,taken,lot_avail, memo)
       
        else:
            subject = subjects_left[0]
            #This code adds the class to the subject and continues on
            withclass, classestaken = self.helper(subjects_left[1:],
                                         work_avail - subject.getWork(),taken+(subject,), lot_avail-subject.getLottery(),
                                         memo)
            withclass += subject.getValue()
            #This code doesn't take the class and sees what happens
            withoutclass, withoutclass_classes = self.helper(subjects_left[1:],
                                                    work_avail,taken,lot_avail, memo)
            #This class evaluates whether value is maximized with the class
            #or without it
