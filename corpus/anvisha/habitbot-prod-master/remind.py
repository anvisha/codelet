'''
Last updated: 21 Nov. Fixing tabs after yesterday's rad pair programming session.
This should be updated to include checking if the goal is done today or not.
'''
...
def remind_sender(scheduled_item):

    #check to see if message still accurate
    stored_tod = scheduled_item.goal.remind_tod
    stored_dow = scheduled_item.goal.remind_dow
    ...
    if message_obj == None or message_obj.message_state != 'PENDING':
        created_msg = Message.objects.create(message_state = 'PENDING',
                               ...
                               message_sender = Participants.objects.get(twitterid = 'habitbot'),
                               num_tries = 0,
                               message_text = scheduled_item.goal.remindmsg,
                               scheduled_by = scheduled_item.id)
    return str(created_msg+' remind')
