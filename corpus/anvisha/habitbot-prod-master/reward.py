...
def reward_sender(scheduled_item):

    #check to see if message still accurate
    stored_tod = scheduled_item.goal.reward_tod
    stored_dow = scheduled_item.goal.reward_dow
    ...
    if message_obj == None or message_obj.message_state != 'PENDING':
        created_msg = Message.objects.create(message_state = 'PENDING',
                               ...
                               num_tries = 0,
                               message_text = scheduled_item.goal.rewardmsg,
                               scheduled_by = scheduled_item.id)
    return str(created_msg+' reward')
